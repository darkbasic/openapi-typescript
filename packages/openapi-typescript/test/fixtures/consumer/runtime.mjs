...
const schema = {
  ...
  components: {
    schemas: {
      ...
      RequiredCompany: {
        allOf: [{ $ref: "#/components/schemas/Company" }, { type: "object", required: ["name"] }],
      },
      Row: {
        type: "object",
...
for (const immutable of [false, true]) {
  ...
  if (immutable) {
    // The fork deliberately disables raw-key required lowering when a property
    // hook could rename keys. Check allOf separately, retaining both type hooks.
    await writeFile("allof.d.ts", await openapiTS(schema, { ...options, transformProperty: undefined }));
  }
}
