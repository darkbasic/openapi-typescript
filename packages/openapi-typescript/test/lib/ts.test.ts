...
describe("tsWithRequiredObject", () => {
  ...
  test("preserves object semantics", () => {
    const footer: string[] = [];
    tsWithRequiredObject("Source", ["value"], footer);
  ...
  test("keeps named and inline helper semantics equivalent", () => {
    ...
    const aliases = cases
      .map(([name, source]) => {
        const named = tsWithRequiredObject(source, ["value"], footer, false);
        const inline = tsWithRequiredObject(source, ["value"], [], true);
    ...
    const namedGeneric = tsWithRequiredObject(generic, ["value"], footer, false);
    const inlineGeneric = tsWithRequiredObject(generic, ["value"], [], true);
