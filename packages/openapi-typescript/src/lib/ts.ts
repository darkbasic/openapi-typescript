...
// Render named and inline forms from one body so their object constraints stay
// equivalent. The input appears before `infer U`, outside the local binders.
function withRequiredObjectBody(type: TSNode, keyType: TSNode): TSNode {
  return `${tsParenthesize(type)} extends infer U ? unknown extends U ? {
    [P in ${keyType}]-?: unknown;
} : U extends readonly unknown[] ? never : U extends (...args: never[]) => unknown ? never : U extends abstract new (...args: never[]) => unknown ? never : U extends object ? U & {
    [P in ${keyType}]-?: P extends keyof U ? {
        [Q in keyof U]-?: U[Q];
    }[P] : unknown;
} : never : never`;
}

const WITH_REQUIRED_OBJECT = `type WithRequiredObject<T, K extends string | number | symbol> = ${withRequiredObjectBody("T", "K")};`;

export function isWithRequiredObjectHelper(node: FooterDeclaration): boolean {
...
export function tsWithRequiredObject(
  ...
  if (inline) {
    return withRequiredObjectBody(type, keyType);
  }

  // Helper names follow the existing fixed-name injection convention.
  if (!injectFooter.some(isWithRequiredObjectHelper)) {
    injectFooter.push(WITH_REQUIRED_OBJECT);
  }

  return `WithRequiredObject<${type}, ${keyType}>`;
}
